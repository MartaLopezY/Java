 ...
 * 26/1/2023
 *
 * @author marta.lopezyanez Haz un programa con un menú que permita gestionar la
 * cola de espera de un médico. Hay tres tipos de pacientes: los que vienen a
 * consulta (se le pide al usuario nombre, fecha de nacimiento, motivo de la
 * consulta), los que viene por recetas (se le pide: nombre, fecha de
 * nacimiento, lista de medicamentos) y el que viene a revisión (se le pide
 * nombre, fecha de nacimiento y fecha de la visita anterior). • Las tarifas del
 * médico son: Consulta: 100 eur. Recetas: 5 eur por cada unidad. Revisión: 30
 * eur para mayores de 65 años, 50 eur para resto. • Crear una clase para cada
 * tipo de paciente en el propio archivo del programa con los constructores
 * necesarios y el método de facturar() en cada una de las clases. Implementa
 * herencia si lo crees necesario. • El programa tendrá un menú para: a)
 * Registrar la llegada del paciente: se le preguntará por qué viene al médico y
 * se le piden sus datos. b) Llamar a consulta (por orden de llegada). Se le
 * cobra la tarifa correspondiente. c) Consultar total facturado hasta ese
 * momento.
 */
public class MLYt09e09 {
    ...
    public static void registarPaciente() {
        ...
        switch (motivo) {
            case 1 ->
                pacientes.add(new Paciente(nombre, fechaNacimiento, "Consulta"));
            case 2 -> {
